...
mod handler_root_test {
    ...
    #[actix_web::test]
    async fn test_root() {
        let app = test::init_service(App::new().service(root).service(health)).await;
        let req = test::TestRequest::get().uri("/").to_request();
        let res: RootResponse = test::call_and_read_body_json(&app, req).await;
        assert_eq!(res.message, "I AM MENAHEL!");
        assert_eq!(
            res.metadata.as_ref().unwrap().api_version,
    ...
    async fn test_root_with_request_id() {
        ...
        let req = test::TestRequest::get()
            .uri("/")
            .insert_header(("X-Request-ID", uuid.clone().as_str()))
            .to_request();
        let res: RootResponse = test::call_and_read_body_json(&app, req).await;
        assert_eq!(res.message, "I AM MENAHEL!");
