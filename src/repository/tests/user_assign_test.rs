use crate::repository::user_assign_repo::UserAssignRepository;
...
mod user_assign_test {
    ...
    async fn test_user_assign_repo_create_user_assign(pool: SqlitePool) {
        ...
        let created_user_assign = user_assign_repo.create_user_assign(user_assign).await.unwrap();
        assert_eq!(created_user_assign.user_id, 1);
        let retrieved_user_assign = user_assign_repo.get_user_assign_by_id(created_user_assign.id.unwrap()).await.unwrap().unwrap();
    ...
    async fn test_user_assign_repo_get_user_assign_by_id(pool: SqlitePool) {
        ...
        let user_assign = user_assign_repo.get_user_assign_by_id(1).await.unwrap().unwrap();
    ...
    async fn test_user_assign_repo_get_user_assign_by_task_id(pool: SqlitePool) {
        ...
        let user_assign = user_assign_repo.get_user_assign_by_task_id(3).await.unwrap();
    ...
    async fn test_user_assign_repo_get_user_assign_by_task_id_not_found(pool: SqlitePool) {
        ...
        let user_assign = user_assign_repo.get_user_assign_by_task_id(100).await.unwrap();
    ...
    async fn test_user_assign_repo_get_user_assign_by_user_id(pool: SqlitePool) {
        ...
        let user_assign = user_assign_repo.get_user_assign_by_user_id(1).await.unwrap();
        assert_eq!(user_assign.len(), 2);
        for user_assign in user_assign {
            assert_eq!(user_assign.user_id, 1);
        }
    ...
    async fn test_user_assign_repo_get_user_assign_by_user_id_not_found(pool: SqlitePool) {
        ...
        let user_assign = user_assign_repo.get_user_assign_by_user_id(100).await.unwrap();
        assert_eq!(user_assign.len(), 0);
    }
    ...
    async fn test_user_assign_repo_get_user_assign_by_user_id_and_task_id(pool: SqlitePool) {
        ...
        let user_assign = user_assign_repo.get_user_assign_by_user_id_and_task_id(1, 3).await.unwrap().unwrap();
        assert_eq!(user_assign.user_id, 1);
        assert_eq!(user_assign.task_id, 3);
    }

    #[sqlx::test(fixtures("user_assign"))]
    async fn test_user_assign_repo_get_user_assign_by_user_id_and_task_id_not_found(pool: SqlitePool) {
        let user_assign_repo = UserAssignRepository::new(pool);

        let user_assign = user_assign_repo.get_user_assign_by_user_id_and_task_id(1, 100).await.unwrap();
        assert!(user_assign.is_none());
    }
    ...
    async fn test_user_assign_repo_update_user_assign(pool: SqlitePool) {
        ...
        let updated_user_assign = user_assign_repo.update_user_assign(user_assign).await.unwrap();
