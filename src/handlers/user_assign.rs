...
async fn get_user_assigns_with_pagination(
    ...
    match pagination_params.status() {
        PaginationStatus::Active => {
            ...
            log::debug!(
                "Getting user assigns with pagination: page: {:?}, page_size: {:?}",
                pagination_params.page(),
                pagination_params.page_size()
            );
            user_assign_repo
                .get_user_assigns_by_filter(
                    filter,
                    pagination_params.page(),
                    pagination_params.page_size(),
                )
        ...
        PaginationStatus::Inactive => {
            ...
            user_assign_repo
                .get_user_assigns_by_filter(filter, None, None)
                .await
                .map_err(HandlerError::from)
        }
...
async fn get_all_or_filtered_user_assigns(
    ...
    let result = get_user_assigns_with_pagination(
        &pagination_params,
        query.get_user_assign_filter().as_ref(),
        pool,
    )
    .await;
...
async fn get_user_assign_by_id(
    req: HttpRequest,
    query: GetUserAssignsQuery,
    pool: SqlitePool,
) -> HttpResponse {
    ...
    let user_assign = user_assign_repo
        .get_user_assign_by_id(validated_query.id.unwrap())
        .await
        .map_err(HandlerError::from);
...
async fn get_user_assigns(
    ...
    match target {
        QueryTarget::All => {
            get_all_or_filtered_user_assigns(req, query, pool.get_ref().clone()).await
        }
...
#[post("/userassigns")]
async fn create_user_assign(
    req: HttpRequest,
    user_assign_data: Result<web::Json<UserAssign>, actix_web::Error>,
    pool: web::Data<SqlitePool>,
) -> HttpResponse {
    let metadata = ResponseMetadata::new(get_request_id(&req));
    ...
    let user_assign = user_assign_repo
        .create_user_assign(user_assign_data.into_inner())
        .await
        .map_err(HandlerError::from);
...
async fn update_user_assign(
    ...
    if user_assign_data.user_assign_id.is_none()
        || (user_assign_data.user_assign_id.is_some()
            && user_assign_data.user_assign_id.unwrap() != path)
    {
        let error = HandlerError::BadRequest(get_error_message(
            ErrorKey::UserAssignHandlerPathAndBodyIdMismatch,
            format!(
                "path.id: {:?}, user_assign_data.id: {:?}",
                path, user_assign_data.user_assign_id
            ),
        ));
        let response = ErrorResponse::new(error.to_string(), 1, None);
        return handle_error(error, response);
    ...
    let user_assign_repo = UserAssignRepository::new(pool.get_ref().clone());
    let user_assign = user_assign_repo
        .update_user_assign(user_assign_data.into_inner())
        .await
        .map_err(HandlerError::from);
